...
const budgets = require("../data").budget;
    ...
    bills.getBillByMonth(req.user.username, req.params.month - 1, parseInt(req.params.year))
        .then(b => {
            let g = Enumerable.from(b)
                ...
                let g1 = Enumerable.from(b)
                .groupBy(b => b.category)
                .select(a1 => {
                    console.log("b:"+JSON.stringify(b))
                    return{
                        category: a1.key(),
                        spending: a1.sum(i=>i.amount)
                    }; 
                }).toArray();
                 g1.forEach(function(value){
                    budgets.updateSpending(req.user.username, value.spending, req.params.month - 1, parseInt(req.params.year), value.category);
                });
